...
Mat Optimizer::tail(Mat img, string index) {
  if (index == "f") {
    ...
    Mat border(img.rows - 450, img.cols, cropped_image_f.type(),
               Scalar(0, 0, 0));
    Mat dst_front;
    ...
    Mat border2(img.rows, img.cols - 450, cropped_image_l.type(),
                Scalar(0, 0, 0));
    Mat dst_left;
    ...
    Mat border1(img.rows - 400, img.cols, cropped_image_b.type(),
                Scalar(0, 0, 0));
    Mat dst_behind;
    ...
    Mat border3(img.rows, img.cols - 450, cropped_image_r.type(),
                Scalar(0, 0, 0));
    Mat dst_right;
...
void Optimizer::distortPointsOcam(cv::Mat &P_GC1, cv::Mat &p_GC,
...
void Optimizer::distortPoints(cv::Mat &P_GC1, cv::Mat &p_GC,
                              Eigen::Matrix3d &K_C) {
  for (int i = 0; i < P_GC1.cols; i++) {
...
void Optimizer::initializePose() { // ground->camera
  ...
  left_disturbance_rot_euler << 2.9, -2.6, 1.0;
  ...
  Mat_<double> behind_disturbance_t =
      (Mat_<double>(3, 1) << 0.095, 0.099, 0.079);
...
void Optimizer::fine_Calibrate_left(int search_count, double roll_ep0,
  ...
  fine_random_search_params(search_count, roll_ep0, roll_ep1, pitch_ep0,
                            ...
                            t1_ep1, t2_ep0, t2_ep1, "left", "less");
}
...
void Optimizer::fine_Calibrate_right(
  ...
  fine_random_search_params(search_count, roll_ep0, roll_ep1, pitch_ep0,
                            pitch_ep1, yaw_ep0, yaw_ep1, t0_ep0, t0_ep1, t1_ep0,
                            t1_ep1, t2_ep0, t2_ep1, "right", "less");
}
...
void Optimizer::fine_Calibrate_behind(
  ...
  fine_random_search_params(search_count, roll_ep0, roll_ep1, pitch_ep0,
                            pitch_ep1, yaw_ep0, yaw_ep1, t0_ep0, t0_ep1, t1_ep0,
                            t1_ep1, t2_ep0, t2_ep1, "behind", "less");
}
...
double Optimizer::CostFunction(const vector<double> var, string idx,
  ...
  if (idx == "right") {
    ...
    Tr *= deltaT;
    double loss =
        back_camera_and_compute_loss(imgf_bev, imgr_gray, Tr, "fr", model, hr);
    return loss;
  } else if (idx == "left") {
    ...
    Tl *= deltaT;
    double loss =
        back_camera_and_compute_loss(imgf_bev, imgl_gray, Tl, "fl", model, hl);
    return loss;
  } else {
    ...
    Tb *= deltaT;
    double loss =
        back_camera_and_compute_loss(imgl_bev, imgb_gray, Tb, "bl", model, hb);
    loss +=
        back_camera_and_compute_loss(imgr_bev, imgb_gray, Tb, "br", model, hb);
    return loss;
  }
}
...
void Optimizer::random_search_params(int search_count, double roll_ep0,
  ...
  if (idx == "left") {
    imgl_bev = project_on_ground(imgl_gray, extrinsic_left, intrinsic_left,
                                 distortion_params_left, KG, brows, bcols, hl);
    imgl_bev_rgb =
        project_on_ground(imgl_rgb, extrinsic_left, intrinsic_left,
                          distortion_params_left, KG, brows, bcols, hl);
    imgl_bev = tail(imgl_bev, "l");
    imgl_bev_rgb = tail(imgl_bev_rgb, "l");
  } else if (idx == "right") {
    imgr_bev = project_on_ground(imgr_gray, extrinsic_right, intrinsic_right,
                                 distortion_params_right, KG, brows, bcols, hr);
    imgr_bev_rgb =
        project_on_ground(imgr_rgb, extrinsic_right, intrinsic_right,
                          distortion_params_right, KG, brows, bcols, hr);
...
void Optimizer::fine_random_search_params(
  ...
  if (idx == "left") {
    ...
    imgb_bev =
        project_on_ground(imgb_gray, extrinsic_behind, intrinsic_behind,
                          distortion_params_behind, KG, brows, bcols, hr);
    imgb_bev_rgb =
        project_on_ground(imgb_rgb, extrinsic_behind, intrinsic_behind,
                          distortion_params_behind, KG, brows, bcols, hr);
    imgb_bev = tail(imgb_bev, "b");
