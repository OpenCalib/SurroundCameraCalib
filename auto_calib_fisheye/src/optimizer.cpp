...
void Optimizer::initializePose() {//ground->camera
    ...
    T_FG<< 9.99277118e-01,  3.82390286e-04, -3.80143958e-02,6.75437418e-01,
	      ...
	      -3.02124625e-02,  6.14928921e-01, -7.88003572e-01,3.17779305e+00,
		  0,0,0,1;
    T_LG<< -1.21898860e-02,  9.99924056e-01, -1.81349393e-03,1.36392943e+00,
	      ...
	      -5.96711036e-01, -8.72966581e-03, -8.02408707e-01,1.04105913e+01,
		  0,0,0,1;
    T_BG<<-9.99615699e-01,  1.56439861e-02, -2.28849354e-02,1.09266953e+00,
	       ...
	       9.64060983e-03, -5.77827838e-01, -8.16101739e-01,6.60957845e+00,
		   0,0,0,1;
    T_RG<< 4.57647596e-03, -9.99989102e-01,  9.22798184e-04,-1.66115120e-01,
		  -6.26343448e-01, -3.58584197e-03, -7.79538984e-01,1.76226207e+01, 
	       7.79533797e-01,  2.98955282e-03, -6.26353033e-01,6.08338205e+00,
		   0,0,0,1;

    Eigen::Matrix4d front_disturbance;
	Eigen::Matrix3d front_disturbance_rot_mat;
	Vec3f front_disturbance_rot_euler;//R(euler)
	Mat_<double> front_disturbance_t=(Mat_<double>(3, 1)<<0.07,0.08,-0.093);
	...
	left_disturbance_rot_euler<<1.02,-2.98,0.55;
	...
	right_disturbance_rot_euler<<-2.965,0.22,0.83;
    ...
    cout<<"eular:"<<endl<<TransformUtil::Rotation2Eul(extrinsic_left.block(0,0,3,3))<<endl;
    cout<<"extrinsic_right:"<<endl<<extrinsic_right<<endl;
    cout<<"eular:"<<endl<<TransformUtil::Rotation2Eul(extrinsic_right.block(0,0,3,3))<<endl;
    cout<<"extrinsic_behind:"<<endl<<extrinsic_behind<<endl;
    cout<<"eular:"<<endl<<TransformUtil::Rotation2Eul(extrinsic_behind.block(0,0,3,3))<<endl;
    return;
}
...
Optimizer::Optimizer(const Mat *imgf,const Mat *imgl,const Mat *imgb,const Mat *imgr,int camera_model_index,int rows,int cols,string fixed_,int flag){
	imgf_rgb = *imgf;
...
void Optimizer::Calibrate_front(int search_count,double roll_ep0,double roll_ep1,double pitch_ep0,double pitch_ep1,double yaw_ep0,double yaw_ep1,
                            double t0_ep0,double t0_ep1,double t1_ep0,double t1_ep1,double t2_ep0,double t2_ep1) {
    vector<double> var(6, 0);
    string varName[6] = {"roll", "pitch", "yaw", "tx", "ty", "tz"};
...
void Optimizer::fine_Calibrate_front(int search_count,double roll_ep0,double roll_ep1,double pitch_ep0,double pitch_ep1,double yaw_ep0,double yaw_ep1,
                            double t0_ep0,double t0_ep1,double t1_ep0,double t1_ep1,double t2_ep0,double t2_ep1) {
    fine_random_search_params(search_count,
        roll_ep0,roll_ep1,pitch_ep0,pitch_ep1,yaw_ep0,yaw_ep1,
        t0_ep0,t0_ep1,t1_ep0,t1_ep1,t2_ep0,t2_ep1,
...
double Optimizer::back_camera_and_compute_loss(Mat img1,Mat img2,Eigen::Matrix4d T,string idx){
	...
	for(int i=0;i<size;i++){
	    ...
	    if(x1>0&&y1>0&&x1<img2.cols&&y1<img2.rows){
	    	loss+=fabs(getPixelValue(&img1,x,y)/ncoef-getPixelValue(&img2,x1,y1));
			// if(idx=="rf")
...
void Optimizer::random_search_params(int search_count, 
    ...
    double resolution_r=150;
    double resolution_t=150;
	...
	for (size_t i = 0; i < search_count; i++) {
		...
		if(idx=="front"&&loss_new<cur_front_loss){//if fix back camera,front camera is calibrated at last
