...
extractor::extractor(Mat img1_bev_,Mat img2_bev_,int flag_){
	img1_bev=img1_bev_;
	img2_bev=img2_bev_;
	flag=flag_;
	Mat bev;
...
extractor::extractor(Mat img1_bev_,Mat img2_bev_,
              Mat intrinsic1_,Mat intrinsic2_,
...
vector<cv::Point> extractor::extrac_textures_and_save(string filename,string idx,double size){
	int down_sample=500;
    Mat gray1,gray2;
    ...
    for(auto pixel:contour1_pixels){
        if ( pixel.x < 10 || pixel.y < 10 || ( pixel.x+10 ) >img1_bev.cols || ( pixel.y+10 ) >img1_bev.rows )
            continue;	
		if(flag&&(abs(pixel.x-camera_center_x)>(size-20)||abs(pixel.y-camera_center_y)>(size-20)))
			continue;
...
vector<cv::Point> extractor::extrac_textures(string idx,double size){
	int down_sample=500;
    Mat gray1,gray2;
    cvtColor(img1_bev,gray1,COLOR_BGR2GRAY);
    cvtColor(img2_bev,gray2,COLOR_BGR2GRAY);
    ...
    for(auto pixel:contour1_pixels){
        if ( pixel.x < 10 || pixel.y < 10 || ( pixel.x+10 ) >img1_bev.cols || ( pixel.y+10 ) >img1_bev.rows )
            continue;	
		if(flag&&(abs(pixel.x-camera_center_x)>(size)||abs(pixel.y-camera_center_y)>(size)))
			continue;
        Eigen::Vector2d delta (
            (gray1.ptr<uchar>(pixel.y)[pixel.x] - gray1.ptr<uchar>(pixel.y)[pixel.x-2]), 
            (gray1.ptr<uchar>(pixel.y)[pixel.x] - gray1.ptr<uchar>(pixel.y-2)[pixel.x]));
