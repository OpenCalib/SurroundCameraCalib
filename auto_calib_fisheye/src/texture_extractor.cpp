...
#include<fstream>
#include<algorithm>
extractor::extractor(Mat img1_bev_,Mat img2_bev_,int edge_flag_,int exposure_flag_,vector<double>size){
	img1_bev=img1_bev_;
	img2_bev=img2_bev_;
...
vector<pair<cv::Point, double>> extractor::extrac_textures_and_save(string pic_filename,string csv_filename){
	int down_sample=800;
    Mat gray1,gray2;
    cvtColor(img1_bev,gray1,COLOR_BGR2GRAY);
    cvtColor(img2_bev,gray2,COLOR_BGR2GRAY);
    // adaptiveThreshold(gray1, gray1, 255, ADAPTIVE_THRESH_GAUSSIAN_C, THRESH_BINARY, 17, -3);  
    // adaptiveThreshold(gray2, gray2, 255, ADAPTIVE_THRESH_GAUSSIAN_C, THRESH_BINARY, 17, -3);  
	if(exposure_flag){
		Mat ROI1,ROI2;
    ...
    // adaptiveThreshold(show, show, 255, ADAPTIVE_THRESH_GAUSSIAN_C, THRESH_BINARY, 15, -2);  
    // GaussianBlur(show,show,Size(5,5),0);	
	...
	for(int i=0;i<texture.size();i++){
        ...
        int k=rand()%texture.size();
		if(local_pixel_test(texture_down,texture[k]))	
			continue;
...
bool extractor::local_pixel_test(vector<pair<cv::Point, double>>texture,pair<cv::Point, double> pixel){
