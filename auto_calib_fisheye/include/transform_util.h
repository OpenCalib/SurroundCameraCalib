...
public:
    ...
    {
        Eigen::Matrix4d ret = Eigen::Matrix4d::Identity();
        ret << matrix(0), matrix(4), matrix(8), matrix(12), matrix(1),
    ...
    static Eigen::Matrix3d eulerAnglesToRotationMatrix(
        Vec3f &theta)  // theta为角度制
    {
        // 转化为弧度制
        ...
        Mat R_y = (Mat_<double>(3, 3) << cos(th1), 0, sin(th1), 0, 1, 0,
                   -sin(th1), 0, cos(th1));

        // Calculate rotation about z axis
        Mat R_z = (Mat_<double>(3, 3) << cos(th2), -sin(th2), 0, sin(th2),
                   cos(th2), 0, 0, 0, 1);
