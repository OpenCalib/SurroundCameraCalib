...
public:
    ...
    // Optimizer();
    Optimizer(const Mat *imgf,const Mat *imgl,const Mat *imgb,const Mat *imgr,int camera_model_index,int rows,int cols,string first_order,int flag,string data_set,int flag_add_disturbance,string prefix,string solution_model_);
    ~Optimizer();   
    void initializeK();
    void initializeD();
    void initializePose();    
    ...
    double CostFunction(const vector<double> var,string idx,Eigen::Matrix4d T);
    ...
    void fine_random_search_params(int search_count, 
                                    ...
                                    double t0_ep0,double t0_ep1,double t1_ep0,double t1_ep1,double t2_ep0,double t2_ep1,
                                    string idx);
    void best_random_search_params(int search_count, 
									double roll_ep0,double roll_ep1,double pitch_ep0,double pitch_ep1,double yaw_ep0,double yaw_ep1,
                                    double t0_ep0,double t0_ep1,double t1_ep0,double t1_ep1,double t2_ep0,double t2_ep1,
                                    string idx);
    ...
    Mat project_on_ground(Mat img, Eigen::Matrix4d T_CG,
						  Eigen::Matrix3d K_C,vector<double> D_C,
						  Eigen::Matrix3d K_G,int rows, int cols, float height);
    double back_camera_and_compute_loss(Mat img1_bev_gray,Mat img1_bev_atb,Mat img2_gray,Mat img2_atb,Eigen::Matrix4d T,string idx);
